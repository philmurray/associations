...
module.exports = function(bookshelf){
	var Game = bookshelf.Model.extend({
		tableName: "games",
		players: function() {
			return this.belongsToMany(models.User).withPivot('completed', 'start_time', 'current_word', 'score', 'normal');
		},
		words: function() {
					...
					model.related('players').forEach(function(player){
						...
						if (user && user.id === player.id){
							returnModel.player = {
								...
								currentWord: player.pivot.get('current_word'),
		...
		updateGame: function(user, gameId, options, callback){
			...
			return new Game({id: gameId}).fetch({withRelated: [
				...
				var player = model.related('players').models[0];
				if (options.startGame){
					if (!player || player.pivot.get('completed') || player.pivot.get('start_time')){
						return callback(new Error('Game could not be started'));
					}
					return model.related('players').updatePivot({'start_time': new Date()}, {query: userQuery}).then(function(){
						//TODO: start a timer to 'complete the game after 30 seconds'
						return model.related('words').filter(function(word){return word.pivot.get('order') === player.pivot.get('current_word');})[0];
					});
				}
			})
