	...
	var Game = bookshelf.Model.extend({
		...
		getGames: function(user, callback){
			return new Game().query(function(qb){
				qb.whereRaw("id in (select game_id from games_users where user_id = '" + user.id + "' and declined = false)");
			...
			.then(function(games){
				...
				games.forEach(function(game){
					...
					if (g.status === Game.GAME_STATUS.Completed || g.status === Game.GAME_STATUS.Waiting){
						var map = {};
