...
module.exports = function(bookshelf){
	var Game = bookshelf.Model.extend({
		tableName: "games",
		players: function() {
			return this.belongsToMany(models.User).withPivot(['completed']);
		},
		...
		getGame: function(user, gameId, callback){
			return new Game({id: gameId}).fetch({withRelated: ['topWords', 'scoredPlayers', 'scoredPlayers.color', {'scoredPicks':function(qb){
				...
				.then(function(model){
					...
					returnModel.players = model.related('scoredPlayers').map(function(player, index){
						...
						if (player === activePlayer){
							...
							returnModel.level = player.get('level');
							returnModel.levelProgress = player.get('level_progress');
							fullModel = true;
						}
						if (fullModel){
		...
		completeGame: function(game_id){
			...
			return new Game({id: game_id, completed: false}).fetch({withRelated: ['picks']}).then(function(game){
				if (!game) return null;
		...
		getPlayerHighScore: function(user, callback){
			return new Game().query(function(qb){
		...
		getPlayerMostPicks: function(user, callback){
			return new Game().query(function(qb){
