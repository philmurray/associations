...
module.exports = function(bookshelf){
	...
	var GameUser = bookshelf.Model.extend({
		...
		completeGame: function(gameUserQuery){
				...
				return ScoredGameUser.get(gameUserQuery).then(function(model){
					if (model.running){
						return new models.Pick({
							...
							to: null,
							user_id: gameUserQuery.user_id,
							game_id: gameUserQuery.game_id,
							time_taken: 0
						})
			...
			.then(function(){
				new GameUser().where({game_id:gameUserQuery.game_id}).fetchAll()
					.then(function(gameUsers){
						if (gameUsers.every(function(model){ return model.get('completed');})){
							return models.Game.completeGame(gameUserQuery.game_id);
						}
					});
				return null; //don't need to wait on game completion
			});
		},
