...
module.exports = function(bookshelf){
	...
	var GameUser = bookshelf.Model.extend({
		...
		completeGame: function(user, gameId){
			...
			.then(function(game){
				if (game.players.every(function(player){return player.completed;})){
					models.Game.completeGame(gameId);

					var players = game.players.filter(function(player){return player.winner;}).map(function(player){ return player.alias;});
					if (players.length) {
						models.Chat.addGameCompletedChat(gameId, players);
					}
				}
				return game;
			});
