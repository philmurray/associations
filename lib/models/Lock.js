...
module.exports = function(bookshelf){
	var Lock = bookshelf.Model.extend({
		...
		playerTime: function(user){
			return new models.UserStats.Time({user_id: user.id}).fetch();
		},
		playerRank: function(user){
			return new models.UserStats.Rank({user_id: user.id}).fetch();
		}
	});
	return Lock;
};
